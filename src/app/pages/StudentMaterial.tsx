...
const StudentMaterial: React.FC = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const mData = await apiGet<{ materials: Material[] }>('/api/materials');
        setMaterials(mData.materials);
        if (!user?.id) {
          setRequests([]);
          return;
        }
        const rData = await apiGet<{ requests: Request[] }>(
          `/api/material-requests?studentId=${encodeURIComponent(user.id)}`
        );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await apiSend('/api/material-requests', 'POST', newRequest);
      const rData = await apiGet<{ requests: Request[] }>(
        `/api/material-requests?studentId=${encodeURIComponent(user.id)}`
      );
      setRequests(rData.requests);
      ...
      toast.success('Demande envoyée avec succès', {
        description: "L'administrateur va examiner votre requête.",
      });
    } catch (err) {
