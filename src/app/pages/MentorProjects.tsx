...
const MentorProjects: React.FC = () => {
  ...
  useEffect(() => {
    void (async () => {
      try {
        const u = await apiGet<{ users: User[] }>('/api/users');
        setUsers(u.users);
      } catch {
        setUsers([]);
      }
  ...
  const updateProject = async (
    ...
    try {
      await apiSend(`/api/projects/${encodeURIComponent(projectId)}`, 'PATCH', updates);
      await loadProjects();
  ...
  const handleUpdateFeedback = async () => {
    ...
    if (await updateProject(selectedProject.id, { mentorFeedback: feedback }))
  ...
  const handleUpdateProgress = async (progressValue: string) => {
    if (!selectedProject) return;
    if (await updateProject(selectedProject.id, { progress: parseInt(progressValue, 10) }))
