    ...
    const load = async () => {
      try {
        const [uData, pData] = await Promise.all([
          apiGet<{ users: { role: string; approved: boolean }[] }>('/api/users'),
          apiGet<{ projects: { status: string }[] }>('/api/projects'),
        ]);
        ...
        setStats({
          ...
          activeStudents: users.filter((u) => u.role === 'student' && u.approved).length,
          activeMentors: users.filter((u) => u.role === 'mentor' && u.approved).length,
        });
      } catch {
        /* keep zeros */
      }
